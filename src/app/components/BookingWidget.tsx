...
export default function BookingWidget({ onClose }: Props) {
          ...
          <AnimatePresence mode="wait">
            ...
            {step === "calendar" && (
              <motion.div key="calendar" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.25, ease }} className="p-5">
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="font-stolzl text-[15px] font-semibold text-[#02022c]">
                ...
                <div className="grid grid-cols-7 mb-2">
                  {days.map((d) => (
                    <div key={d} className="text-center font-stolzl text-[12px] text-[#5c5c5c] py-1">{d}</div>
                  ))}
                </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          {available && !past && !selected && (
                            <span className="absolute bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-[#335cff]" />
                          )}
                        </button>
                      );
                    })}
            ...
            {step === "time" && selectedDate && (
              <motion.div key="time" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.25, ease }} className="p-5">
                ...
                <p className="font-stolzl text-[15px] font-semibold text-[#02022c] mb-1">
                  {formatDisplayDate(selectedDate, isRTL)}
                </p>
                <p className="font-stolzl text-[13px] text-[#5c5c5c] mb-4">
                  {isRTL ? "اختر وقت الاجتماع (30 دقيقة)" : "Select a 30-minute slot"}
                </p>
            ...
            {step === "success" && (
              <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.4, ease }} className="p-8 flex flex-col items-center text-center">
                <div className="w-16 h-16 rounded-full bg-[#3ab874]/15 flex items-center justify-center mb-5">
                  <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#3ab874" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                    <polyline points="20 6 9 17 4 12" />
                  </svg>
                </div>
                <h3 className="font-stolzl text-[22px] font-bold text-[#02022c] mb-2">
                  {isRTL ? "تم تأكيد الاجتماع!" : "Meeting confirmed!"}
                </h3>
